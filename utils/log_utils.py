...
def prepare_output_and_logger(args, use_tensorboard=True):
    ...
    if use_tensorboard:
        # create folder: model_path/tensorboard/
        tb_root = os.path.join(args.model_path, "tensorboard")
        os.makedirs(tb_root, exist_ok=True)
        # subfolder with timestamp
        timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M")
        tb_log_dir = os.path.join(tb_root, timestamp)
...
def list_parameters(model):
    ...
    for name, value in model.__dict__.items():
        ...
        elif isinstance(value, torch.nn.ParameterList):
            for i, p in enumerate(value):
                params.append((f"{name}.{i}", p))
    return params