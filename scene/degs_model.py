...
class DeGSModel(GaussianModel):
    ...
    def capture(self):
        ...
        return parent_data + (self.change_ids, self._internal_mask, self._internal_log_scale, )
    ...
    def add_internal_gaussians_and_freeze_others(
        ...
        for oid in change_ids:
            ...
            new_rotation[:, 0] = 1.0
            # ===== internal latent scale bookkeeping =====
            if self._internal_log_scale is None:
                # 第一次添加 internal GS
                self._internal_log_scale = new_log_scale_1d
            else:
                # 后续多次 add internal GS
                self._internal_log_scale = torch.cat(
                    [self._internal_log_scale, new_log_scale_1d], dim=0
                )
            # -------- opacity --------
        ...
        with torch.no_grad():
            ...
            self._internal_log_scale.requires_grad_(True)
    ...
    def project_internal_scale_to_scaling(self):
        if self._internal_log_scale is None:
            return

        with torch.no_grad():
            mask = self._internal_mask
            s = self._internal_log_scale
            self._scaling[mask] = s.repeat(1, 3)