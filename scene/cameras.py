...
class Camera(nn.Module):
    def __init__(self, colmap_id, R, T, FoVx, FoVy, image, gt_alpha_mask, 
                 ...
                 data_device="cuda", depth=None, 
                 semantic_feature=None, id_mask=None, target_mask=None):
        ...
        if semantic_feature is not None:
            self.semantic_feature = semantic_feature.to(self.data_device)
        if id_mask is not None:
            self.id_mask = id_mask.to(self.data_device)
        if target_mask is not None:
            self.target_mask = target_mask.to(self.data_device)
    ...
    def load2device(self, data_device='cuda'):
        ...
        self.semantic_feature = self.semantic_feature.to(data_device)
        self.id_mask = self.id_mask.to(data_device)
        self.target_mask = self.target_mask.to(data_device)
        self.depth = self.depth.to(data_device) if self.depth is not None else None
