...
class SamVideoSegmenter:
    ...
    def process_frames(self, frame_paths, prompts_loader, inference_state):
        ...
        while True:
            ...
            sum_id = prompts_loader.get_next_id() 
            ...
            if now_frame == 0:
                ...
                new_mask_list = search_new_obj(
                    masks_from_prev=list(masks_from_prev.values()),
                    mask_list=masks,
                    mask_ratio_thresh=mask_ratio_thresh
                )
                for obj_id, m in masks_from_prev.items():
                    if m.sum() > 0:
                        prompts_loader.add(obj_id, now_frame, m[0])
            ...
            #         break
            for out_frame_idx in range(now_frame, len(frame_paths), self.args.detect_stride):
                seg_dict = video_segments[out_frame_idx]          # {obj_id: mask}
                out_mask_list = list(seg_dict.values())           # 给 cal_no_mask_area_ratio 用
                no_mask_ratio = cal_no_mask_area_ratio(out_mask_list)
                ...
                if no_mask_ratio > mask_ratio_thresh + 0.01 and out_frame_idx > now_frame:
                    masks_from_prev = seg_dict                    # ✅ 保留 obj_id
                    max_area_no_mask = (no_mask_ratio, out_frame_idx)
                    break
    ...
    def save_results(self, frame_names, video_segments, stage=0, offset=0):
        ...
        for local_idx, frame_name in enumerate(frame_names):
            ...
            seg_dict = video_segments[global_idx]
            out_obj_id_list = sorted(seg_dict.keys())
            out_mask_list   = [seg_dict[i] for i in out_obj_id_list]
            save_idmasks(out_obj_id_list, out_mask_list, local_idx, save_dir)
