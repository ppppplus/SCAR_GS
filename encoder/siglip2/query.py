...
def main(img_folder, cache_dir, query_text, feature_extractor, mask_generator, feature_compressor):
    img_names = sorted(os.listdir(img_folder))
    data = np.load(os.path.join(cache_dir, "cache.npy"), mmap_mode="r")
    id_masks = np.load(os.path.join(cache_dir, "segmaps.npy"))
    resize_if_taller_than = 1080
    text_feat = feature_extractor.extract_text_features(query_text)
    # text_prob = tokenizer([query_text], context_length=model.context_length).to(device)
    ...
    for idx, img_name in enumerate(img_names):
        ...
        # id_mask, ids = sam_masks_to_id_mask(masks[3], order="score", background_id=-1, start_id=0)
        id_mask = id_masks[idx]
        features, cursor = get_image_features(data, id_mask, cursor)
        print(cursor)
        # id_mask = np.load(id_mask_path)
        # # print(len(np.unique(id_mask)))
        seg_feats = torch.from_numpy(features).float().to(device)
        # # print(seg_feats.shape)

        prob_map1 = text_image_match_heatmap(id_mask, seg_feats, text_feat, feature_extractor)
        plot_heatmap(prob_map1, image=img_bgr)
        enc_feats = feature_compressor.encode_vectors(seg_feats)
        # print(img_name, seg_feats.shape)
        prob_map2 = text_image_match_heatmap(id_mask, enc_feats, text_feat, feature_extractor)
        plot_heatmap(prob_map2, image=img_bgr)
        # quantize_features = feature_compressor.encode_vectors(seg_feats)
        # print(quantize_features.shape)
        # prob_map = text_image_match_heatmap(id_mask, quantize_features, text_feat, feature_extractor)
        # plot_heatmap(prob_map, image=img_bgr)
        # visualize_image_and_mask_array(img_bgr, id_mask, image_is_bgr=True)
        # break

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Training script parameters")
    parser.add_argument('--source_path', type=str, default="/home/ubuntu/TJH/Work/aff_ws/LEGaussians/datasets/mipnerf360/bicycle")
    parser.add_argument('--ckpt_name', type=str, default="siglip2_sam05_0000_128_768_1_20251013-205649_codebook.pt")
    parser.add_argument('--query', type=str, default="bicycle")
    parser.add_argument('--n_e', type=int, default=128)
    parser.add_argument('--e_dim', type=int, default=768)
    parser.add_argument('--beta', type=float, default=0.)

    args = parser.parse_args()
    img_folder = os.path.join(args.source_path, "images")
    cache_dir = os.path.join(img_folder, "siglip2_features_saml")
    ...
    sam_ckpt_path = "/home/ubuntu/TJH/Work/aff_ws/langscenex_ws/LangScene-X/ckpt/sam_vit_h_4b8939.pth"
    feature_extractor = SIGLIP2NetWork(device=device)
    mask_generator = build_sam(sam_ckpt_path, device=device)

    feature_compressor = VectorQuantizer(args.n_e, args.e_dim, args.beta, device, concat=False)
    sd = torch.load(os.path.join(img_folder, args.ckpt_name), map_location=device)
    missing, unexpected = feature_compressor.load_state_dict(sd, strict=True)
    print("missing:", missing, "unexpected:", unexpected)
    feature_compressor.to(device).eval() 
    main(img_folder, cache_dir, args.query, feature_extractor, mask_generator, feature_compressor)
