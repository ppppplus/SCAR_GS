		...
		bool include_feature)
{
	...
	float last_semantic_feature[F] = {0};
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		if (range.x + progress < range.y)
		{
			...
			for (int i = 0; i < F; i++)
				collected_feature[i * BLOCK_SIZE + block.thread_rank()] = semantic_feature[coll_id * F + i];
		}
		block.sync();
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			if (include_feature)
			{
				for (int ch = 0; ch < F; ch++)
				{
					...
					// Update last color (to be used in the next iteration)
					accum_rec_F[ch] = last_alpha * last_semantic_feature[ch] + (1.f - last_alpha) * accum_rec_F[ch];
					...
					// Atomic, since this pixel is just one of potentially
					// many that were affected by this Gaussian.
					atomicAdd(&(dL_dsemantic_feature[global_id * F + ch]), dchannel_dcolor * dL_dchannel_F);
				}
			}
