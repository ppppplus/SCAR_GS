...
__global__ void __launch_bounds__(BLOCK_X *BLOCK_Y)
	renderCUDA(
		...
		float *__restrict__ out_semantic_feature,
		bool include_feature)
{
	...
	float C[CHANNELS] = {0};
	float F[CHANNELS_semantic_feature] = {0};
	float weight = 0;
	float D = 0;
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			if (include_feature)
			{
				for (int ch = 0; ch < CHANNELS_semantic_feature; ch++)
					F[ch] += semantic_feature[collected_id[j] * CHANNELS_semantic_feature + ch] * alpha * T;
			}
	...
	if (inside)
	{
		...
		{
			for (int ch = 0; ch < CHANNELS_semantic_feature; ch++)
				out_semantic_feature[ch * H * W + pix_id] = F[ch]; // bg_color ???
		}
	}
}
